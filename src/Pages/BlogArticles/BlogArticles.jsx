...
const BlogArticles = () => {
  ...
  const handleReadMore = async (blog) => {
    ...
    await axios.patch(`http://localhost:5000/blogs/${blog._id}`, {
  ...
  return (
    ...
    <div className="min-h-screen px-4 py-8 bg-gradient-to-br from-indigo-25 to-violet-50">
        ...
        {blogs.map((blog) => (
          ...
          >
            ...
            <div className="p-5 space-y-3">
              ...
              <h2 className="text-xl font-bold text-indigo-900">{blog.title}</h2>
      ...
      >
        <div className="fixed inset-0 bg-indigo-900/80 backdrop-blur-sm" aria-hidden="true" />
          ...
          <Dialog.Panel className="w-full max-w-2xl mx-auto rounded-2xl bg-white shadow-2xl overflow-hidden border border-indigo-100">
            {selectedBlog && (
              <div className="p-6">
                ...
                <p className="text-gray-700 mb-6">{selectedBlog.fullDetails}</p>
                
                <div className="flex justify-between items-center pt-4 border-t border-indigo-100">
                  <div className="flex items-center gap-2">
                    <FaUserCircle className="text-indigo-500" />
                    <span className="font-medium text-indigo-700">{selectedBlog.author}</span>
                  </div>
                  <Link
                  ...
                  >
                    Full Policy Brief <FaArrowRight className="text-xs mt-0.5" />
                  </Link>
                </div>
              </div>
            )}
