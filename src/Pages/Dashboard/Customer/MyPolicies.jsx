...
useEffect(() => {
 ...
 const handleSubmitReview = async (rating, feedback) => {
  if (selectedPolicy) {
    ...
    try {
      // POST to backend
      const res = await axios.post("http://localhost:5000/testimonials", newTestimonial);

      if (res.data.insertedId || res.data.acknowledged) {
        // Update policy as reviewed in frontend state
        const updatedPolicies = policies.map(policy =>
          policy._id === selectedPolicy._id ? { ...policy, hasReview: true } : policy
        );
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-10 text-center">
        <h1 className="text-3xl md:text-4xl font-bold text-gray-800 mb-3">My Insurance Policies</h1>
        <p className="text-gray-600 max-w-2xl mx-auto">
          Manage your insurance policies, view details, and submit reviews for your purchased policies.
        </p>
      </div>
