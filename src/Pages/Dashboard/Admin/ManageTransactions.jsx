...
const ManageTransactions = () => {
  ...
  const filteredTransactions = transactions.filter(t => {
    ...
    const matchesSearch = t.email.includes(search) || 
  ...
  return (
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      <div className="flex flex-col md:flex-row justify-between gap-4 mb-6">
        <div>
          <h2 className="text-2xl md:text-3xl font-bold text-gray-800">Transaction Management</h2>
                      ...
                      <Menu.Item>
                        {({ active }) => (
                          <button
                            ...
                            className={`${
                              active ? 'bg-gray-100' : ''
                            } block w-full text-left px-4 py-2 text-sm ${filter === "all" ? "text-indigo-600 font-medium" : "text-gray-700"}`}
                          >
                            All Statuses
                          </button>
                        )}
                      ...
                      <Menu.Item>
                        {({ active }) => (
                          <button
                            ...
                            className={`${
                              active ? 'bg-gray-100' : ''
                            } block w-full text-left px-4 py-2 text-sm ${filter === "completed" ? "text-indigo-600 font-medium" : "text-gray-700"}`}
                          >
                            Completed
                          </button>
                        )}
                      ...
                      <Menu.Item>
                        {({ active }) => (
                          <button
                            ...
                            className={`${
                              active ? 'bg-gray-100' : ''
                            } block w-full text-left px-4 py-2 text-sm ${filter === "pending" ? "text-indigo-600 font-medium" : "text-gray-700"}`}
                          >
                            Pending
                          </button>
                        )}
                      ...
                      <Menu.Item>
                        {({ active }) => (
                          <button
                            ...
                            className={`${
                              active ? 'bg-gray-100' : ''
                            } block w-full text-left px-4 py-2 text-sm ${filter === "failed" ? "text-indigo-600 font-medium" : "text-gray-700"}`}
                          >
                            Failed
                          </button>
                        )}
              ...
              {filteredTransactions.map((t) => (
                <tr key={t.transactionId} className="hover:bg-gray-50 transition">
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="hidden md:inline text-sm font-medium text-gray-900">#{t.transactionId}</span>
                    <span className="md:hidden text-sm font-medium text-gray-900">#{t.transactionId.slice(0, 8)}...</span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm font-medium text-gray-900">{t.email}</div>
                    <div className="text-sm text-gray-500 hidden md:block">User ID: {t.userId}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="text-sm text-gray-900 max-w-[160px] truncate">{t.policyName}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right">
                    <div className="text-sm font-semibold text-gray-900">${(t.amount / 100).toFixed(2)}</div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium ${statusColors[t.status.toLowerCase()] || 'bg-gray-100 text-gray-800'}`}>
                      {t.status}
                    </span>
                  </td>
